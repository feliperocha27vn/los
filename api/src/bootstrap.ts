...
async function applyFile(filename: string): Promise<void> {
  ...
  // se dois processos (ex.: um deploy sobrepondo o anterior) tentarem aplicar o
  // mesmo arquivo ao mesmo tempo, o perdedor da corrida não deve derrubar o
  // processo, só constatar que já foi registrado.
...
export async function runMigrations() {
  ...
  if (count === 0) console.log('[bootstrap] no new migrations')
  else console.log(`[bootstrap] ${count} migration(s) applied`)
}

export async function runSeed() {
  console.log('[bootstrap] checking seed...')
  ...
  if (result.seeded) {
    console.log('')
    console.log('========================================')
    console.log(`[bootstrap] USER CREATED: ${result.email}`)
    console.log(`[bootstrap] password: ${process.env.ADMIN_PASSWORD || '12345678'}`)
    console.log(`[bootstrap] cofre pin: ${process.env.ADMIN_PIN || '123456'}`)
    console.log('========================================')
    console.log('')
  } else {
    console.log(`[bootstrap] user already exists (${result.email}), skipped seed`)
  }
}

export function reportStartupError(e: unknown) {
  ...
  if (err.cause?.code === '28P01' || err.message?.includes('password authentication')) {
    console.error('')
    console.error('==========================================================')
    console.error('[bootstrap] FATAL: PostgreSQL authentication failed')
    console.error('')
    console.error('Check DATABASE_URL. Locally this usually means the persistent')
    console.error('volume los-postgres-data was initialized with a different')
    console.error('POSTGRES_PASSWORD than the one currently in the env vars.')
    console.error('')
