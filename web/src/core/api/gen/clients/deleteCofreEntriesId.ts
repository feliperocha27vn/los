 ...
 */
export async function deleteCofreEntriesId(id: DeleteCofreEntriesIdPathParams["id"], config: Partial<RequestConfig> & { client?: Client } = {}) {
  const { client: request = fetch, ...requestConfig } = config



  const res = await request<DeleteCofreEntriesIdMutationResponse, ResponseErrorConfig<DeleteCofreEntriesId404>, unknown>({ method : "DELETE", url : getDeleteCofreEntriesIdUrl(id).url.toString(), ... requestConfig })
  return res.data
}