 ...
 */
export async function getCofreEntriesId(id: GetCofreEntriesIdPathParams["id"], config: Partial<RequestConfig> & { client?: Client } = {}) {
  const { client: request = fetch, ...requestConfig } = config



  const res = await request<GetCofreEntriesIdQueryResponse, ResponseErrorConfig<GetCofreEntriesId404>, unknown>({ method : "GET", url : getGetCofreEntriesIdUrl(id).url.toString(), ... requestConfig })
  return res.data
}