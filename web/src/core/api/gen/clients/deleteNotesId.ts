 ...
 * {@link /notes/:id}
 */
export async function deleteNotesId(id: DeleteNotesIdPathParams["id"], config: Partial<RequestConfig> & { client?: Client } = {}) {
  const { client: request = fetch, ...requestConfig } = config



  const res = await request<DeleteNotesIdMutationResponse, ResponseErrorConfig<DeleteNotesId404>, unknown>({ method : "DELETE", url : getDeleteNotesIdUrl(id).url.toString(), ... requestConfig })
  return res.data
}